...
let stripeSecretKey: any;
let stripeWebhookSecret: any;
let openaiApiKey: any;
let stripePriceIdTutela: any;
...
if (process.env.USE_ENV_VARS !== 'true') {
  try {
    ...
    stripePriceIdTutela = defineSecret("STRIPE_PRICE_ID_TUTELA");
  } catch (error) {
...
const resolveOpenAIKey = (): string => {
  ...
  throw new Error(
    "OPENAI_API_KEY is not configured. Set it as a secret (firebase functions:secrets:set OPENAI_API_KEY) or environment variable."
  );
};
...
const resolveStripeSecretKey = (): string => {
  ...
  throw new Error(
    "STRIPE_SECRET_KEY is not configured. Set it as a secret or environment variable."
  );
};
...
const resolveStripeWebhookSecret = (): string => {
  ...
  throw new Error(
    "STRIPE_WEBHOOK_SECRET is not configured. Set it as a secret or environment variable."
  );
};
...
const FONT_REGULAR_PATH = path.join(FONT_DIR, "SourceSans3-Regular.otf");
const FONT_BOLD_PATH = path.join(FONT_DIR, "SourceSans3-Bold.otf");
  ...
  const paragraphs = splitTextIntoParagraphs(options.body).map((block) => {
    ...
    const children = lines.map((line, index) =>
      index === 0
        ...
        : new TextRun({ text: line, break: 1, size: 24 })
    );
    ...
    sections: [
      {
          ...
          default: new Header({
            children: [
              new Paragraph({
                ...
                children: [
                  new TextRun({ text: "Avocat LegalTech", bold: true, size: 28 }),
                  new TextRun({
        ...
        children: [
          new Paragraph({
            ...
            alignment: AlignmentType.CENTER,
            children: [new TextRun({ text: options.title, size: 36, bold: true })],
            spacing: { after: 320 },
          }),
          options.subtitle
            ? new Paragraph({
                alignment: AlignmentType.CENTER,
                children: [new TextRun({ text: options.subtitle, size: 24, italics: true })],
                spacing: { after: 320 },
              })
            : undefined,
          ...paragraphs,
...
const createPdfBuffer = (options: {
  ...
  return new Promise<Buffer>((resolve, reject) => {
    try {
      ...
      const addText = (text: string, fontSize: number, isBold: boolean = false, color: string = "#111827", align: "left" | "center" | "right" = "left") => {
      ...
      const drawHeader = () => {
        ...
        doc.text(
          "Centro de Documentos Jurídicos y Formación Profesional para Estudiantes",
          pageWidth / 2,
          headerY + 6,
          { align: "center" }
        );
...
function buildStoragePath(
  uid: string,
  userType: "abogado" | "estudiante" | "autoservicio",
  documentType: string,
...
const uploadToStorage = async (buffer: Buffer, path: string, contentType: string) => {
...
type StorageUploadResult = Awaited<ReturnType<typeof uploadToStorage>>;

interface StudentDocumentPackageInput {
  userId: string;
...
interface StudentDocumentPackageCoreResult {
  packageId: string;
  storageBasePath: string;
  files: {
    templateDocx: StorageUploadResult;
    templatePdf: StorageUploadResult;
    sampleDocx: StorageUploadResult;
    samplePdf: StorageUploadResult;
    studyMaterialPdf: StorageUploadResult;
  };
}

// Helper function to handle CORS
const onRequestWithCors = (handler: any) => {
...
const onRequestWithCorsAndSecrets = (options: any, handler: any) => {
...
// Test function
export const helloWorld = onRequestWithCors((req: any, res: any) => {
...
async function extractSingleWithOpenAI(
  openai: OpenAI,
  documentText: string,
  fileName: string,
  pageLabel?: string,
  excelStructure?: string
): Promise<{ country: string; documentType: string; emisor: string; receptor: string; fields: { key: string; value: string }[] }> {
...
- Usa "-" cuando no encuentres un valor. Máximo 25 campos. Responde SOLO con el JSON.`;
  ...
  const parsed = JSON.parse(jsonStr) as { country?: string; documentType?: string; emisor?: string; receptor?: string; fields?: { key: string; value: string }[] };
  return {
...
async function extractMultiWithOpenAI(
  openai: OpenAI,
  documentText: string,
  fileName: string,
  pageLabel: string,
  excelStructure?: string
): Promise<Array<{ country: string; documentType: string; emisor: string; receptor: string; fields: { key: string; value: string }[] }>> {
...
Reglas CRÍTICAS: Extrae SOLO datos del texto. NO inventes. NO infieras del nombre del archivo. Emisor = vendedor, Receptor = cliente. Identifica TODAS las facturas/recibos (pueden ser 3, 4 o más en una página). ${fieldsHint} Usa "-" si no hay valor. Responde SOLO con el JSON.`;
  ...
  const content = completion.choices[0]?.message?.content || "{}";
  const jsonStr = content.replace(/```json\s*/g, "").replace(/```\s*/g, "").trim();
  const parsed = JSON.parse(jsonStr) as { documents?: Array<{ country?: string; documentType?: string; emisor?: string; receptor?: string; fields?: { key: string; value: string }[] }> };
...
import { processInvoiceWithDocumentAI } from "./document-ai";
...
export const extraccionDatosExtract = onRequestWithCorsAndSecrets({
  ...
  memory: "1GiB",
    ...
    try {
      ...
      if (preExtractedTextPerPage && Array.isArray(preExtractedTextPerPage) && preExtractedTextPerPage.length > 0) {
        ...
        if (pageTexts.length === 0) pageTexts = [preExtractedText?.trim() || ""].filter(Boolean);
      }
      ...
      if (pagesToProcess.length === 1) {
        ...
        if (textIsEmpty && isPdf) {
          res.json({ ...result, totalPages: pageTexts.length || 1, needsClientOcr: true, pageTexts });
        } else {
          res.json(result);
        }
        return;
      ...
      const results: Array<{ country: string; documentType: string; emisor: string; receptor: string; fields: { key: string; value: string }[]; pageIndex?: number; subIndex?: number }> = [];
      for (let i = 0; i < pagesToProcess.length; i++) {
        ...
        if (multiInvoicePerPage && pageText.trim().length > 100) {
          ...
          multiResults.forEach((r, idx) => results.push({ ...r, pageIndex: pageNum, subIndex: multiResults.length > 1 ? idx + 1 : undefined }));
        } else {
      ...
      if (results.length === 0 && pagesToProcess.length > 0) {
        ...
        res.json({
          ...fallback,
          totalPages: hasEmptyPages ? pageTexts.length : 1,
          needsClientOcr: hasEmptyPages && pageTexts.length > 1,
          pageTexts: hasEmptyPages ? pageTexts : undefined,
        });
        return;
      }
      ...
      if (!preExtractedTextPerPage?.length && pageTexts.length > 0) {
...
export const testOpenAI = onRequestWithCors(async (req: any, res: any) => {
  try {
    const openai = createOpenAIClient();
    const completion = await openai.chat.completions.create({
      model: "gpt-4o",
...
export const reclamacionCantidades = onRequestWithCors(async (req: any, res: any) => {
  if (req.method !== "POST") {
    res.status(405).json({ error: "Method not allowed" });
    return;
  }
  ...
  try {
    ...
    const docRef = await db.collection("reclamaciones").add({
      ...
      fechaFin,
      salarioBruto,
      documento: completion.choices[0]?.message?.content || "",
...
export const accionTutela = onRequestWithCors(async (req: any, res: any) => {
  if (req.method !== "POST") {
    res.status(405).json({ error: "Method not allowed" });
    return;
  }
...
export const getUserStats = onRequestWithCors(async (req: any, res: any) => {
  try {
    const db = admin.firestore();
...
export const disableUser = onRequestWithCors(async (req: any, res: any) => {
  try {
    const { uid } = req.body;
...
export const enableUser = onRequestWithCors(async (req: any, res: any) => {
  try {
    const { uid } = req.body;
...
export const cleanupUser = onRequestWithCors(async (req: any, res: any) => {
  try {
    const { uid } = req.body;
...
export const updateUserStatus = onRequestWithCors(async (req: any, res: any) => {
  try {
...
export const checkUserStatusByEmail = onRequestWithCors(async (req: any, res: any) => {
  try {
...
export const reactivateUser = onRequestWithCors(async (req: any, res: any) => {
  try {
    const { uid } = req.body;
...
export const getUserStatus = onRequestWithCors(async (req: any, res: any) => {
  try {
...
export const analyzeDocuments = onRequestWithCors(async (req: any, res: any) => {
  if (req.method !== "POST") {
    res.status(405).json({ error: "Method not allowed" });
    return;
  }
  ...
  try {
    ...
    let tutelaId: string | null = null;
    let reclId: string | null = null;
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      bb.on('field', (name, value) => {
        ...
        if (name === 'tutelaId') tutelaId = value;
        if (name === 'reclId') reclId = value;
      });
...
export const createCheckoutSession = onRequestWithCorsAndSecrets({
  ...
  try {
    ...
    console.log('Currency determination:', {
      documentType,
      firstItemCountry: items?.[0]?.country,
    ...
    let tutelaPriceId: string | undefined; // Declarar fuera del bloque para usar en validación
    ...
    console.log('Creating Stripe checkout session with:', {
      ...
      lineItems: lineItems.map(li => ({
        ...
        total: (li.price_data?.unit_amount || 0) * (li.quantity || 1)
      })),
      customerEmail,
    ...
    const stripeRequestPayload: Stripe.Checkout.SessionCreateParams = {
      payment_method_types: ['card'],
...
const generateStudentDocumentPackageCore = async ({
  userId,
  userEmail,
  areaLegal,
  tipoEscrito,
...
}: StudentDocumentPackageInput): Promise<StudentDocumentPackageCoreResult> => {
      ...
      [
        {
          role: "system",
          content:
            "Eres un abogado senior especializado en formación universitaria. Elaboras plantillas jurídicas exhaustivas, impecables y listas para personalizar.",
        },
        {
          role: "user",
      ...
      [
        {
          role: "system",
          content:
            "Actúas como abogado tutor. Redactas ejemplos completos con información ficticia verosímil para guiar a estudiantes de derecho.",
        },
        {
          role: "user",
      ...
      [
        {
          role: "system",
          content:
            "Eres catedrático universitario de derecho. Elaboras guías académicas profundas, estructuradas y listas para estudio.",
        },
        {
          role: "user",
  ...
  const [
    ...
    uploadToStorage(
      templateDocx,
      `${basePath}/plantilla/plantilla-${documentSlug}.docx`,
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    ),
    uploadToStorage(
      templatePdf,
      `${basePath}/plantilla/plantilla-${documentSlug}.pdf`,
      "application/pdf"
    ),
    uploadToStorage(
      sampleDocx,
      `${basePath}/ejemplo/ejemplo-${documentSlug}.docx`,
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    ),
    uploadToStorage(
      samplePdf,
      `${basePath}/ejemplo/ejemplo-${documentSlug}.pdf`,
      "application/pdf"
    ),
    uploadToStorage(
      studyPdf,
      `${basePath}/estudio/dossier-${documentSlug}.pdf`,
      "application/pdf"
    ),
  ]);
...
export const generateStudentDocumentPackage = onRequestWithCors(async (req: any, res: any) => {
  if (req.method !== "POST") {
...
async function handleStripeWebhook(req: any, res: any) {
  ...
  const stripe = new Stripe(resolveStripeSecretKey(), { apiVersion: "2023-10-16" });
  ...
  try {
    ...
    // Method 5: Last resort - try to reconstruct from parsed object (will fail signature verification)
      ...
      const underlyingReq = (req as any).raw || req;
      if (underlyingReq && underlyingReq.readable) {
        const chunks: Buffer[] = [];
        for await (const chunk of underlyingReq) {
          chunks.push(chunk);
        }
        rawBody = Buffer.concat(chunks);
...
webhookApp.use((req: any, res, next) => {
  if (req.method !== 'POST') {
    return next();
  }
...
async function generateTutelaDocumentPackageCore({
  userId,
  userEmail,
  formData,
  tutelaId,
  ...
  storageBasePath: string;
  files: {
    tutelaPdf: StorageUploadResult;
    tutelaDocx: StorageUploadResult;
  };
}> {
...
El documento debe ser profesional, completo, y listo para presentación ante el juez competente. Debe tener una extensión mínima de 2,000 palabras y seguir el formato estándar de acciones de tutela en Colombia.`;
  ...
  const tutelaResult = await runChatCompletion(
    openai,
    [
      {
        role: "system",
  ...
  const [tutelaPdfUpload, tutelaDocxUpload] = await Promise.all([
    uploadToStorage(
      tutelaPdf,
      `${basePath}/accion-tutela-${docId}.pdf`,
      "application/pdf"
    ),
    uploadToStorage(
      tutelaDocx,
      `${basePath}/accion-tutela-${docId}.docx`,
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    ),
  ]);
...
async function generateReclamacionDocumentPackageCore({
  userId,
  userEmail,
  caseId,
  uid,
...
${caseData.ocr.rawText?.substring(0, 2000) || 'No hay resumen disponible del OCR.'}
...
Redacta el escrito en español de España, con formato claro, párrafos bien separados y sin instrucciones para el usuario final. No incluyas explicaciones sobre lo que estás haciendo; solo devuelve el texto del escrito.`;
  ...
  const reclamacionResult = await runChatCompletion(
    openai,
    [
      {
        role: 'system',
  ...
  const [reclamacionPdfUpload, reclamacionDocxUpload] = await Promise.all([
    uploadToStorage(
      reclamacionPdf,
      `${basePath}/reclamacion-cantidades-${caseId}.pdf`,
      "application/pdf"
    ),
    uploadToStorage(
      reclamacionDocx,
      `${basePath}/reclamacion-cantidades-${caseId}.docx`,
      "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
    ),
  ]);
  console.log('✅ Archivos subidos a Storage');
  ...
  try {
    ...
    try {
      ...
      if (!uploadsSnapshot.empty) {
        ...
        if (uploadData.uploadedFiles && Array.isArray(uploadData.uploadedFiles)) {
          ...
          uploadedDocuments = uploadData.uploadedFiles.map((file: any) => ({
    ...
    const quantityPromises = Array.from({ length: item.quantity }, (_, i) => {
      return generateTutelaDocumentPackageCore({
        ...
        storagePath: generation.files.tutelaPdf?.path || null,
        downloadUrl: generation.files.tutelaPdf?.downloadUrl || null,
        generatedAt: admin.firestore.Timestamp.now(),
        packageFiles: {
    ...
    const firstDocPackageFiles = generatedDocuments.length > 0 && generatedDocuments[0]?.packageFiles 
    ...
    const itemResult = {
      ...
      generatedAt: admin.firestore.Timestamp.now(),
      status: generatedDocuments.length > 0 ? 'completed' : 'failed',
      documents: generatedDocuments,
      generatedCount: generatedDocuments.length,
      requestedCount: item.quantity,
      documentType: 'accion_tutela' as const,
      tutelaId: tutelaId,
      docId: docId,
      formData: formData,
    };
...
async function processWebhookAsync(event: Stripe.Event) {
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      let tutelaId = session.metadata?.tutelaId || null;
      let docId = session.metadata?.docId || null;
      let caseId = session.metadata?.caseId || null;
      let uid = session.metadata?.uid || null;
      let formDataJson = session.metadata?.formData;
      ...
      if (!itemsJson) {
        ...
        try {
          let metadataSnapshot: admin.firestore.QuerySnapshot | null = null;
      ...
      if (!itemsJson) {
        ...
        try {
          ...
          if (expandedSession.line_items && expandedSession.line_items.data && expandedSession.line_items.data.length > 0) {
            ...
            const constructedItems = lineItems.map((lineItem: any) => {
              const price = lineItem.price;
              const amount = price?.unit_amount || 0;
              const quantity = lineItem.quantity || 1;
              
              // Determinar documentType basado en el nombre del producto
              const productName = price?.product?.name || lineItem.description || '';
      ...
      const itemsStatus: any[] = [...purchaseData.items];
      let completedCount = 0;
      
      // Helper function to update Firestore with current progress
      const updateProgress = async () => {
        const currentDocumentsGenerated = itemsStatus.filter((item: any) => item.status === 'completed').length;
      ...
      if (!purchaseData.items || !Array.isArray(purchaseData.items) || purchaseData.items.length === 0) {
        ...
        if (items && Array.isArray(items) && items.length > 0) {
          ...
          purchaseData.items = items.map((item: any) => ({
            id: uuidv4(),
            name: item.name,
            area: item.area,
            country: item.country,
      ...
      const itemPromises = purchaseData.items.map((item: any, index: number) => 
        processItemDocuments(item, index)
      );
      ...
      const totalDocumentsGenerated = updatedItems.reduce((sum: number, item: any) => {
