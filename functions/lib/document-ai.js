...
exports.processInvoiceWithDocumentAI = processInvoiceWithDocumentAI;
const v1_1 = require("@google-cloud/documentai/build/src/v1");
...
function inferCountryFromTaxId(taxId) {
    ...
    if (spanishNif.test(t) || spanishDni.test(t) || spanishNie.test(t))
        return "España";
    if (/^[A-Z]{2}/.test(t))
        return "Extranjero";
    return "";
 ...
 */
function mapDocumentToResult(document) {
    var _a;
    ...
    for (const entity of document.entities) {
        ...
        const value = entity.mentionText || ((_a = entity.normalizedValue) === null || _a === void 0 ? void 0 : _a.text) || "";
        if (!value.trim())
            continue;
        ...
        if (type === "supplier_name" || type === "vendor_name")
            emisor = value.trim();
        if (type === "receiver_name" || type === "remit_to_name")
            receptor = value.trim();
...
function mapToPozueloFields(result) {
    const byKey = new Map();
    for (const f of result.fields) {
        const pozueloKey = TO_POZUELO_KEY[f.key] || f.key;
        if (f.value || !byKey.has(pozueloKey))
            byKey.set(pozueloKey, f.value);
    }
    const ordered = [];
    ...
    for (const [k, v] of byKey) {
        if (!POZUELO_FIELD_ORDER.includes(k) && v)
            ordered.push({ key: k, value: v });
    }
    return Object.assign(Object.assign({}, result), { fields: ordered });
}
/**
 * Process a PDF buffer with Document AI Invoice Parser.
 * Returns null if processor is not configured or processing fails.
 */
async function processInvoiceWithDocumentAI(buffer, _fileName, mimeType = "application/pdf", excelStructure) {
    ...
    try {
        ...
        const client = new v1_1.DocumentProcessorServiceClient({ apiEndpoint });
