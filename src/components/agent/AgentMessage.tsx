...
export default function AgentMessage({ message }: { message: Message }) {
  ...
  const showDownload = !isUser && !message.streaming && message.content.length > 150;
  ...
  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-4`}>
      ...
      <div className={`max-w-[75%] ${isUser ? 'order-1' : ''}`}>
          ...
          <div className="flex flex-wrap gap-2 mb-1.5 justify-end">
            {message.attachments.map((att, i) =>
              att.mimeType.startsWith('image/') ? (
                ...
                <span key={i} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-avocat-gold/10 border border-avocat-gold/20 text-[11px] font-sans text-avocat-gold">
                  📎 {att.name}
                </span>
              )
            )}
        ...
        {/* Download + copy row on completed assistant messages */}
        {showDownload && (
