...
export default function DocTable({ docs, cases, clients, selected, onToggleSelect, onSelectAll, sortCol, sortDir, onSort, previewDocId, onAction }: Props) {
  ...
  const openMenu = (e: React.MouseEvent<HTMLButtonElement>, doc: DocumentRecord) => {
    e.stopPropagation();
        ...
        <table className="w-full text-[12px] font-sans border-collapse">
          <thead>
            <tr className="border-b border-[#2e2b20]">
              ...
              <th className={`${TH} hidden md:table-cell`} onClick={() => onSort('status')}>Estado<SortIcon col="status" /></th>
              <th className="px-3 py-2.5 w-8" />
            </tr>
          </thead>
          <tbody className="divide-y divide-[#2e2b20]">
            {docs.map(d => {
              ...
              const displayExt = d.pdfDownloadUrl ? 'pdf' : d.type;
              ...
              return (
                ...
                >
                  ...
                  <td className={`${TD} hidden lg:table-cell text-[#6b6050] max-w-[160px]`}>
                    {caseDoc
                      ? <span className="truncate block">{caseDoc.title}</span>
      ...
      {menuDocId && menuDoc && (
        ...
        >
          {[
            ...
            <button key={action} onClick={() => { setMenuDocId(null); if (action === 'rename') startRename(menuDoc); else onAction(action as DocAction, menuDoc); }}
              className="w-full text-left px-4 py-2 text-[12px] text-[#c8c0ac] hover:bg-[#252218] hover:text-[#e8d4a0] transition-colors">
              {label}
            </button>
          ))}
          <div className="border-t border-[#2e2b20] mt-1 pt-1">
            <button onClick={() => { setMenuDocId(null); onAction('delete', menuDoc); }}
              className="w-full text-left px-4 py-2 text-[12px] text-red-400 hover:bg-red-500/5 transition-colors">
              Eliminar
            </button>
          </div>
        </div>
